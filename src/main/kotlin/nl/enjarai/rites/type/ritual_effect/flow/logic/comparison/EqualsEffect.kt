...
class EqualsEffect(val left: InterpretedNumber, val right: InterpretedNumber) : RitualEffect(CODEC) {
    companion object {
        val CODEC: Codec<EqualsEffect> = RecordCodecBuilder.create { instance ->
            instance.group(
                InterpretedNumber.CODEC.fieldOf("left").forGetter { it.left },
                InterpretedNumber.CODEC.fieldOf("right").forGetter { it.right }
            ).apply(instance, ::EqualsEffect)
        }
    }
